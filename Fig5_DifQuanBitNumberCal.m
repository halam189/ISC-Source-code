...
cSystemParam 	= {};		%cell contains system parameter that defined by some settings or calculated through training phase
	%index of data in the cell 'cSystemParam'
        ...
        nTrainOtherIdx  = 10;
%++++<MO0021>
        nDimNoIdx       = 11;       %number of dimension
%----<MO0021>
...
for iCodeWord = 1 : length (vnCodeWordSize)
    ...
    if  cSystemParam{nCodeWordIdx} == 255
        strFileName = '_BCHKey\\255\\38U\\(255,79,27,38)eKey';
    else
        strFileName = '_BCHKey\\511\\38U\\(511,139,54,38)eKey';
    end
	...
	for iQuan = vnQuanBitNum(1): vnQuanBitNum(2)
		cSystemParam{nQuanBitNoIdx} = iQuan;
		...
		%TESTING DATA PROCESSING
			...
			if (cSystemParam{nCodeWordIdx} == 127)
				mbDecodedKey = func_BCHDecode(mbDebindTestingTempAUs,127,50,13);
			elseif  (cSystemParam{nCodeWordIdx} == 255)
				mbDecodedKey = func_BCHDecode(mbDebindTestingTempAUs,255,79,27);
			elseif  (cSystemParam{nCodeWordIdx} == 511)
				mbDecodedKey = func_BCHDecode(mbDebindTestingTempAUs,511,139,54);
            end
		...
		mnHisIntra = [mnHisIntra; vnHammingIntra];
			mnHisInter = [mnHisInter; vnHammingInter];
			
		end
		strFolderPath = '_TempGaitData\\';
	%strFileName = strcat('FEATURE_',num2str(iCodeWord),'DIFFE_QUANBITNUM');
    strFileName = 'HAMMING_DISTANCE_OF_DIFFERENT_BIT_NUM_';
	strFilePath = strcat(strFolderPath,strFileName);
	strFileNameIntra = strcat(strFilePath,'Intra');
	strFileNameInter = strcat(strFilePath,'Inter');
	
	fileIDTRA = fopen(strFileNameIntra,'w');
	fileIDTER = fopen(strFileNameInter,'w');
	
	for(iRow = 1:(size(mnHisIntra,1)))
		fprintf(fileIDTRA,'%d ',mnHisIntra(iRow,:)); 
		fprintf(fileIDTRA,'\n');
		fprintf(fileIDTER,'%d ',mnHisInter(iRow,:)); 
		fprintf(fileIDTER,'\n');
	end	
	fclose(fileIDTRA);
	fclose(fileIDTER);
