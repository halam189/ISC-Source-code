    ...
    for i=1:n
        ...
        if q3 > q1
            rectangle('Position',[i-widthl/2+(2*j-1-widths)*widthl/(2*d) q1 widths*widthl/d q3-q1],...
                'EdgeColor',colors{j,i},'FaceColor',[ 1 1 1],'HitTest','off','Parent',hgg(j));
        end
...
%             'HitTest','off','Parent',hgg(j));
        ...
        line([i-widthl/2+(2*j-1-widths)*widthl/(2*d) i-widthl/2+(2*j-1+widths)*widthl/(2*d)] , [u u],...
            'Color',colors{j,i},'LineWidth',1,'HitTest','off','Parent',hgg(j));
        %hold off;
    end
end
...
box on;
 ...
 %hamming case
%   set(gca,'XTick',1:3:n);
%   set(gca,'XTickLabel',labels(1:3:end));
...
% % both hamming & euc case
%  set(gca,'XTick',[1:2]);
%  set(gca,'XTickLabel',{'Real valued templates','Reliable binary templates'});
